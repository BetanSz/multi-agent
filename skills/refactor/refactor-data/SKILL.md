...
**Only run this skill if a code change (from `refactor-structure` or otherwise) has affected the schema or semantics of data already stored in Cosmos DB, Blob Storage, or any persistent store.** If no stored data is affected, this skill does not run.
