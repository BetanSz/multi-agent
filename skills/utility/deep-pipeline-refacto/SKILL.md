...
description: Architectural refactor skill for Python/Azure data pipelines. Phase 1 — systematic audit for agentic coding antipatterns (dead code, client over-instantiation, spaghetti flow, silent failures, prompt drift, missing idempotency, abstraction inconsistency, N+1 calls, boilerplate overkill). Phase 2 — executes structural changes, not afraid of large-scale redesign. Phase 3 (conditional) — data migration protocol when code changes make stored data stale. Use when code was generated by multiple agents across multiple sprints and needs a holistic quality pass.
...
Work through findings in severity order. For each:
