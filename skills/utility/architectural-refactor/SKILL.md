...
> **Using skill architectural-refactor.**
...
**Freedom level: HIGH for structural changes** — judgment on what to restructure and how is yours. You make large changes when the evidence supports them. You do not need permission to rename, move, merge, or dissolve code — only to reprocess stored data (that requires `data-migration`).

**Input:** a findings table from `agentic-antipattern-audit`. If called standalone, run a quick execution flow trace first (entry → call graph → output) before applying the depth lens below.

**Non-goal:** do not touch stored data schemas — that requires the `data-migration` skill. Do not create new skill files or skill folders in the target repository.
...
- Change the schema of stored data — that requires `data-migration`
...
- **Never touch stored data schemas** — hand off to `data-migration` if needed
