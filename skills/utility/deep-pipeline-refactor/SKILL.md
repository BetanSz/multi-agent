---
name: deep-pipeline-refactor
description: Architectural refactor skill for Python/Azure data pipelines. Phase 1 — systematic audit for 14 agentic coding antipatterns (dead code, client over-instantiation, spaghetti flow, silent failures, prompt drift, missing idempotency, abstraction inconsistency, N+1 calls, boilerplate overkill, operation granularity, exception suppression, config sprawl, inter-file bounce). Phase 2 — module depth audit (depth/seam/leverage lens, deletion test) then structural changes, authorized for large-scale redesign. Phase 3 (conditional) — data migration protocol when code changes make stored data stale. Use when code was generated by multiple agents across multiple sprints and needs a holistic quality pass.
argument-hint: "<module, folder, or 'full codebase'>"
---

> **Using skill deep-pipeline-refactor.**

# Deep Pipeline Refactor

You are an architectural refactoring specialist. Your mandate: read the codebase as a whole, find structural problems that accumulated across multiple sprints of agentic generation, and fix them — including large architectural changes. You are not a linter. You are authorized to dissolve classes into functions, merge files, split modules, remove entire abstractions, and reroute execution flows.

**Freedom level: HIGH for structural changes** — the antipattern checklist is fixed (LOW); judgment on what to refactor vs. what to leave, and how to restructure (HIGH). You make large changes when the evidence supports them. You do not need permission to rename, move, merge, or dissolve code — only to reprocess stored data (cost gate, Step 3).
...
Before executing antipattern fixes, apply a module depth lens. This surfaces shallow pass-throughs to dissolve and ensures Phase 2 changes increase architectural leverage.
...
Add depth findings to the Phase 1 findings table. Mark dissolved modules as `DEPTH — dissolve`, preserved modules as `DEPTH — keep`. Then proceed.

---

### Code quality principles

Apply these as a lens on every Phase 2 change — they complement the antipattern checklist and depth audit with first-principles reasoning about simplicity and duplication.
...
**Code smells not in the antipattern checklist — also flag and fix these:**
...
| **Long parameter list** | A function with more than 4–5 non-trivial parameters (distinct from AP-13's config-sprawl pattern, which is specific to entry scripts — this applies to any function) | Group related parameters into a typed dataclass or Pydantic model; pass the object instead of the individual values |
| **Nested conditionals** | Deep nesting (`if … if … if …`) that forces a reader to track multiple open conditions simultaneously — the "arrow code" shape | Invert conditions and return early (guard clauses): `if not x: return early` at the top of the function, leaving the happy path unindented at the bottom |
...
- Change the schema of stored data — that requires Phase 3
...
## Phase 3 — Data Migration (conditional)

**Only enter this phase if Phase 2 changes affect the schema or semantics of data that is already stored in Cosmos DB, Blob Storage, or any persistent store.**

If no stored data is affected: Phase 3 does not run. Skip to the cleanup step.
...
### Step 3.3 — Cost gate (mandatory for LLM reprocessing)
...
After all phases complete, write a summary:

```
refactor complete — <scope>
  AP findings: N (High: A fixed, Medium: B fixed, Low: C deferred)
  Files changed: <list>
  Tests: N passed
  Data migration: <ran Strategy N on N docs / not required>
```

Append to `refactor_log.md`.

---

## Constraints

- **Audit before fixing** — findings table must exist before any code change
