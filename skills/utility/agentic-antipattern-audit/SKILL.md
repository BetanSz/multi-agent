---
name: agentic-antipattern-audit
...
> **Using skill agentic-antipattern-audit.**
...
**Non-goal:** do not fix code, do not create skill files, do not modify project structure. Audit and report only. Fixes happen in `architectural-refactor`.
...
**Present the findings table to the user before proceeding.** Do not fix anything. The `architectural-refactor` skill applies the fixes.
...
- **Do not fix anything** — audit only; fixes are in `architectural-refactor`
